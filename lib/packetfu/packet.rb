			...
			if packet.size >= 14													# Min size for Ethernet. No check for max size, yet.
				...
				when "\x08\x00"															# It's IP.
					...
					when 4;				 														# It's IPv4.
						...
						when "\x06"
                            p = TCPPacket.new					# Returns a TCPPacket.
						when "\x11"
                            p = UDPPacket.new					# Returns a UDPPacket.
						when "\x01"
                            p = ICMPPacket.new					# Returns an ICMPPacket.
						else
                            p = IPPacket.new									# Returns an IPPacket since we can't tell the transport layer.
						end
					else
                        p = IPPacket.new										# Returns an EthPacket since we don't know any other IP version.
					end
